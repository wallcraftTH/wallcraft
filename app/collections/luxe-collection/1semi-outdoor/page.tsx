...
// Add your specific display order if needed
const collectionOrder = ['', '', '', '', ''];

const normalizeTitle = (text: string = ''): string => text.trim();

export default function SemiOutdoorCollection() {
    ...
    async function fetchCollection() {
      try {
        ...
        const { data, error } = await supabase
          .from('products')
          .select('*')
          .eq('collection_type', 'semi_outdoor_collection');
  ...
  return (
    <div className="bg-[#0a0a0a] text-white overflow-x-hidden antialiased font-sans">
      <header className="relative w-full h-[90vh] overflow-hidden flex items-center">
        <img
          src="https://raw.githubusercontent.com/WaiHmueThit23/wallcraft_assets/main/luxe_series/Asset%20136@2x.webp"
          alt="Semi Outdoor Collection Hero"
        ...
        <div className="container mx-auto px-8 lg:px-24 relative z-20 pt-20">
          <div className="max-w-2xl animate-reveal">
            ...
            <h1 className="text-4xl lg:text-8xl font-bold tracking-tight mb-6 leading-none drop-shadow-2xl">
              <span className="text-[#B08038]">Semi Outdoor</span>
            </h1>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-[1600px] mx-auto px-8 lg:px-16 py-20 min-h-[50vh]">
        {loading ? (
          <div className="flex flex-col items-center justify-center space-y-4 py-20 opacity-60">
            ...
            <p className="text-[10px] uppercase tracking-[0.2em] text-zinc-500 font-medium">Loading Semi Outdoor Textures...</p>
          </div>
        ) : errorMsg ? (
          ...
          <div className="relative w-full max-w-6xl bg-[#0f0f0f] border border-white/10 rounded-xl overflow-hidden shadow-2xl flex flex-col lg:flex-row max-h-[95vh]">
            ...
            <div className="w-full lg:w-2/5 p-8 lg:p-12 flex flex-col border-l border-white/5 bg-[#0a0a0a] overflow-y-auto no-scrollbar text-left">
              <div className="mb-8">
                ...
                <p className="text-[#c2bfb6] text-[10px] tracking-[0.3em] uppercase mb-4 opacity-80">{currentProduct.subtitle || 'Semi Outdoor Series'}</p>
                ...
                <div className="mb-8">
                  ...
                  <div className="flex flex-wrap gap-3">
                    {currentVariants.map((variant) => {
                      ...
                      return (
                        <button key={variant.id} type="button" onClick={() => openProductModal(variant)} className="group flex flex-col items-center gap-1 cursor-pointer">
